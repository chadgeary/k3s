...
echo "args: --kubelet-arg=provider-id=aws:///$AZ/$INSTANCE_ID --resolv-conf=/etc/rancher/k3s/resolv.conf --node-label=node.kubernetes.io/instance-type=$INSTANCE_TYPE --node-taint=node.cilium.io/agent-not-ready:NoSchedule"
INSTALL_K3S_EXEC="agent --kubelet-arg=provider-id=aws:///$AZ/$INSTANCE_ID --resolv-conf=/etc/rancher/k3s/resolv.conf --node-label=node.kubernetes.io/instance-type=$INSTANCE_TYPE --node-taint=node.cilium.io/agent-not-ready:NoSchedule"
