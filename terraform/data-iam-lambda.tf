...
data "aws_iam_policy_document" "k3s-lambda-oidcprovider" {
  ...
  statement {
    ...
    effect    = "Allow"
    resources = ["arn:${data.aws_partition.k3s.partition}:iam::${data.aws_caller_identity.k3s.account_id}:oidc-provider/${aws_s3_bucket.k3s-public.id}.s3.${var.aws_region}.amazonaws.com*"]
  }
}
