...
resource "aws_ssm_association" "k3s" {
  ...
  parameters = {
    EnvVars         = "AWS_ADDON_URI=${local.aws_addon_uris[var.region]} ACCOUNT=${data.aws_caller_identity.k3s.account_id} REGION=${var.region} PREFIX=${local.prefix} SUFFIX=${local.suffix} DB_ENDPOINT=${aws_db_instance.k3s.endpoint} K3S_NODEGROUP=${each.key} K3S_LB=${aws_lb.k3s-private.dns_name} K3S_URL=https://${aws_lb.k3s-private.dns_name}:6443 SECGROUP=${aws_security_group.k3s-ec2.id} VPC=${aws_vpc.k3s.id} POD_CIDR=${local.pod_cidr} SVC_CIDR=${local.svc_cidr} KUBEDNS_IP=${local.kubedns_ip} VPC_CIDR=${var.vpc_cidr} NAT_GATEWAYS=${tostring(var.nat_gateways)} EBS_KMS_ARN='${aws_kms_key.k3s["ec2"].arn}' EFS_ID=${aws_efs_file_system.k3s.id}"
